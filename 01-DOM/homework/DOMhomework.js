...
createdBy.innerHTML += " Robinson González"; //! <span>nombre</span>
...
function buildToDo(todo, index) {
  ...
  toDoShell.className = "toDoShell"; // Asignarle a 'toDoShell' la clase 'toDoShell' (<div class = 'toDoShell'>)
  let toDoText = document.createElement('span');
  toDoText.innerHTML = todo.description; // <span>description</span>
  toDoText.id = index; // <span id=index>description</span>
...
toDoShell.appendChild(checkbox);
  if (todo.complete) {
    toDoText.className = "completeText"; // Asignarle a 'toDoText' la clase 'completeText'
  }
  toDoShell.appendChild(toDoText); // toDoShell ➡ <div><span>'Una tarea'</span></div>
/*
<div class = 'toDoShell'>
...
function buildToDos(toDos) {
  // Tu código acá:
  let toDoShells = toDos.map(buildToDo); // Tiene que trasnformar el toto en toDoShell //! .map devuelve un array nuevo, distinto
  return toDoShells; // toDoShells ➡ <div><span>'Una tarea'</span></div>
}
...
function displayToDos() {
  ...
  let toDoShells = buildToDos(toDoItems); // Me devuelve un array de toDoShells

  toDoShells.forEach((toDoShell) => { //?     El forEach sirve para recorrer los elementos de un array y ejecutar una funcion 
    toDoContainer.appendChild(toDoShell); //? por cada uno de esos elementos
  })
}
/*
result = [
...
<div class = 'toDoShell'>
<span id=index>description2</span>
</div>
]
*/
...
//  3) Setear el valor del input toDoInput como un string vacio ("") (Esto se realiza para que en la vista se borre 
...
function addToDo() {
  ...
  let Input = new ToDo(toDoInput.value);
...
addButton.addEventListener('enter', addToDo)
...
//   3) En la función' agregar un 'click' event listener al elemento 'toDoText', pasándole
...
/*    Investigá sobre el tipo 'checkbox' del elemento input y realizar lo siguiente en la función 'buildToDo':
        ...
        e) Dentro del bloque 'if' de la función buildToDo, si es true, setear el atributo 'checked' en true en el 
        checkbox
